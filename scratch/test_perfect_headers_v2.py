...
for filename in pdf_files:
    ...
    for h in headers:
        ...
        if start >= last_end:
            ...
            # Filters on the cleaned title:
            # 1. Parentheses check
            if clean_title.count('(') != clean_title.count(')'):
                continue
            ...
            # 3. Comma limits
            if ',' in clean_title:
                if clean_title.count(',') > 1 or len(clean_title) > 50 or len(clean_title.split()) > 6:
                    continue
                    
            # 4. Keyword check
            l_title = clean_title.lower()
            if any(x in l_title for x in ["course objective", "course learning", "evaluation", "laboratory", "text book", "reference", "learning outcomes"]):
                continue
            if re.match(r'^[A-Z]{3,4}\d{3}$', clean_title):
                continue
